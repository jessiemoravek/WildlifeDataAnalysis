...
## This second dataframe is information on each camera trap site. The ID of these sites match up with the ID of the sites in our detection dataframe. This dataframe has data on environmental conditions at each site including the proportion of disturbed land within 5km (Dist_5km), the housing density within 5 km (HDens_5km), the number of people photographed at a site (People_site), and whether the site was on a trail (Trail)

## Filter out a species of interest
bobcat<-filter(meso, Species=="bobcat")

## In order to run occupancy models, we need to format the data in something called an unmarked dataframe. We need to specify our detection/non-detection data (i.e. the 0's and 1's for each site), which is the "y" portion of this function. To do this, we tell R what columns in our data frame have these numbers (in this case it's columns 2-4). We also need to specify the site covariates that we are interested in, which come from our site covariates dataframe. This is done using the "siteCovs" argument in the function - everything within this portion are out site covariates, which we draw from our site_cov dataframe.We are using 1 covariate that we named People, one that we named Dist, one that we named HDesn, and one that we named Trail.
...
## We run occupancy models using the "occu" function from the unmarked package. Within this function, we first specify covariates influencing detection, then covariates influencing occupancy. Let's start with a null model, meaning that nothing is influencing detection or occupancy. When we put "~1" it corresponds with the null model (no covariates on that part of the model)
...
## Now that we know how to run general occupancy model, we are going to use a 2 stage approach to figuring out the top model. We are first going to run models with different detection covariates while keeping occupancy constant.


## One covatiate in this dataset that we might hypothesize might influence detection is the number of people at a site (e.g. maybe more people make animals more illusive). Let's run this model.
